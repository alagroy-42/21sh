...
/*   Updated: 2019/07/23 21:38:31 by pcharrie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	builtin_cd_chdir(char *path, int follow);

int		builtin_cd_chdir_cdpath(char *path, int follow)
{
	char			*str;
	struct	stat	buf;
...
void	builtin_cd_chdir(char *path, int follow)
{
	...
	if (stat(path, &buf) < 0)
	{
		if (builtin_cd_chdir_cdpath(path, follow))
			return ;
...
void	builtin_cd(t_ast *ast, t_env *env)
{
	...
	{
		if (env_get(env, "HOME"))
			builtin_cd_chdir(env_get(env, "HOME")->value, follow);
		else
	...
	else
	{
		...
		else if (!ft_strcmp(ast->args[1], "-"))
		{
			if (env_get(env, "OLDPWD"))
			{
				...
				builtin_cd_chdir(env_get(env, "OLDPWD")->value, follow);
			}
			else
				ft_putstr_fd("cd: OLDPWD not set\n", 2);
		}
		else
			builtin_cd_chdir(ast->args[1], follow);
	}
}
